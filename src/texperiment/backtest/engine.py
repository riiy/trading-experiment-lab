...
from texperiment.backtest.execution_model import can_buy_at_open, has_valid_ohlc
...
def run_stock_rs_pullback_backtest_from_parquet(
    ...
    *,
    ...
    columns = ["date", "code", "open", "high", "low", "close", "is_suspended", "is_limit_up"]
...
def _backtest_one_signal(signal: dict[str, Any], bars: pd.DataFrame | None, cfg: StockRSPullbackBacktestConfig) -> dict[str, Any]:
    ...
    if stop_price is None:
        stop_price = _num(signal.get("pullback_low"))
    if stop_price is None or stop_price >= entry_price:
        row = {**base, "entry_date": _date_str(entry_bar["date"]), "entry_price": entry_price, "stop_price": stop_price}
        return _invalid(row, "invalid_stop_not_below_entry")

    risk = entry_price - stop_price
    target_price = entry_price + risk * cfg.target_r_multiple
    progress_price = entry_price + risk * cfg.time_stop_progress_r_multiple
    horizon = future.iloc[: cfg.max_holding_days].copy().reset_index(drop=True)
    if horizon.empty:
    ...
    for i, (_, row) in enumerate(horizon.iterrows(), start=1):
        bar = row.to_dict()
        if not has_valid_ohlc(bar):
            return _invalid({**base, "entry_date": _date_str(entry_bar["date"]), "entry_price": entry_price, "stop_price": stop_price, "target_price": target_price}, "invalid_missing_price_data")
        ...
        if can_exit_today:
            ...
            if stop_hit and target_hit:
                ...
                if cfg.intraday_priority == "target_first":
                    exit_price = _target_exit_price(bar, target_price)
                    exit_reason = "target_2r"
                else:
                    exit_price = _stop_exit_price(bar, stop_price)
        ...
        if i == cfg.time_stop_days and max_high_since_entry < progress_price:
            exit_date = _date_str(bar["date"])
            exit_price = close
    ...
    ret = compute_trade_return(entry_price, float(exit_price), round_trip_cost=cfg.round_trip_cost)
...
def _invalid(base: dict[str, Any], reason: str, *, entry_bar: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if entry_bar is not None:
        ...
        row["entry_price"] = _num(entry_bar.get("open"))
...
def _stop_exit_price(bar: dict[str, Any], stop_price: float) -> float:
    open_price = float(bar["open"])
    return open_price if open_price <= stop_price else stop_price


def _target_exit_price(bar: dict[str, Any], target_price: float) -> float:
    open_price = float(bar["open"])
    return open_price if open_price >= target_price else target_price
